...
def exercise01():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df = pd.DataFrame(data=[[1, 2, 3, 4], [5, 6, 7, 8], [7, 8, 9, 10]]) 
...
#    Reshape the array. The reshaped array, array_reshaped, should be n/3 columns and 3 rows
    '''
    # ------ Place code below here \/ \/ \/ ------
    r = np.random.randint(0, n-1, n)
    array_1d = np.array(r)
    array_reshaped = array_1d.reshape(3,round(n/3))
...
def exercise12(n):
    ...
    checkerboard_matrix = np.matrix(x).reshape((2*n,2*n))
...
def exercise13(n):
    ...
    index = pd.date_range('20100101', periods=n)
    s = pd.Series(r, index = index).cumsum()
