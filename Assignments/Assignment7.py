...
import chart_studio
chart_studio.tools.set_credentials_file(username='subhalaxmirout', api_key='Ff7s5UJzxiv0QX9hG1cQ')
...
def exercise01():
    ...
    df_first_five_rows = df_iris.head()
...
def exercise02(new_observations):
    ...
    predict = knn.predict(new_observations)
    iris_predictions = iris.target_names[predict]
...
def exercise04():
    ...
    # ------ Place code below here \/ \/ \/ ------
    X_train, X_test, y_train, y_test = tts(iris.data, iris.target, test_size = 0.3, random_state=42, stratify=iris.target)
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x = np.arange(1,31), y = test_score, name = 'train'))
    fig.add_trace(go.Scatter(x = np.arange(1,31), y = train_score, name = 'test'))
    fig.update_layout(
...
def exercise05():
    ...
    average_age = df_boston['AGE'].mean()
    average_medv = np.mean(df_target['MEDV'])
    medv_as_numpy_values = np.array(df_target)
    # ------ Place code above here /\ /\ /\ ------
...
def exercise06():
    ...
    X_ptratio = np.array(df_boston['PTRATIO'])
...
def exercise07():
    ...
    reg = reg.fit(x.values.reshape(-1,1),y.values.reshape(-1,1))
